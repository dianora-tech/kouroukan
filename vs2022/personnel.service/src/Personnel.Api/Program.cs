...
using Personnel.Api.Authorization;
using Personnel.Api.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.Console()
        .Enrich.FromLogContext()
        .Enrich.WithProperty("Application", "personnel-service"));
    ...
    builder.Services.AddGnValidation(builder.Configuration,
        typeof(Personnel.Application.Validators.CreateEnseignantValidator).Assembly);
    ...
    builder.Services.AddMediatR(cfg =>
        cfg.RegisterServicesFromAssembly(typeof(EnseignantCommandHandler).Assembly));
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "Kouroukan - Personnel Service",
            Version = "v1",
            Description = "Microservice de gestion des enseignants, contrats, absences, conges et suivi des heures."
        });
    ...
    Log.Information("Personnel Service demarre sur le port 5006");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Le service Personnel a echoue au demarrage");
}
finally
{
    Log.CloseAndFlush();

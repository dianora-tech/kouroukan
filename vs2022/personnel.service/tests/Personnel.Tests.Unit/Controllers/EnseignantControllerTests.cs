...
public sealed class EnseignantControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<EnseignantController>> _logger;
    private readonly EnseignantController _sut;

    public EnseignantControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<EnseignantController>>();
        _sut = new EnseignantController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Enseignant>(new List<Enseignant>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Enseignant>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Enseignant>>>();
    }
    ...
    {
        var entity = new Enseignant() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Enseignant?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Enseignant() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Enseignant>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Enseignant>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

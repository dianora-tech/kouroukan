...
public sealed class ServiceParentRepository : IServiceParentRepository
{
    private readonly AuditRepository<ServiceParentDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public ServiceParentRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<ServiceParentDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<ServiceParentDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<ServiceParent?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : ServiceParentMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<ServiceParent>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(ServiceParentMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        var spec = new SimpleSpecification<ServiceParentDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(ServiceParentMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<ServiceParent>(entities, result.TotalCount, result.Page, result.PageSize);
    }

    public async Task<ServiceParent> AddAsync(ServiceParent entity, CancellationToken ct = default)
    {
        var dto = ServiceParentMapper.ToDto(entity);
        var created = await _repo.AddAsync(dto, ct).ConfigureAwait(false);
        return ServiceParentMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM services.type_services_parents WHERE is_deleted = FALSE ORDER BY name";

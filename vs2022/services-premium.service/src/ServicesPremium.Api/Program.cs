...
using ServicesPremium.Api.Authorization;
using ServicesPremium.Api.Middleware;
...
using ServicesPremium.Infrastructure.Repositories;
...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.Console()
        .Enrich.FromLogContext()
        .Enrich.WithProperty("Application", "services-premium-service"));
    ...
    builder.Services.AddGnValidation(builder.Configuration,
        typeof(ServicesPremium.Application.Validators.CreateServiceParentValidator).Assembly);
    ...
    builder.Services.AddMediatR(cfg =>
        cfg.RegisterServicesFromAssembly(typeof(ServiceParentCommandHandler).Assembly));

    // 9. Domain Services
    builder.Services.AddScoped<IServiceParentService, ServiceParentService>();
    builder.Services.AddScoped<ISouscriptionService, SouscriptionService>();

    // 10. Repositories
    builder.Services.AddScoped<IServiceParentRepository, ServiceParentRepository>();
    builder.Services.AddScoped<ISouscriptionRepository, SouscriptionRepository>();
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "Kouroukan - Services Premium Service",
            Version = "v1",
            Description = "Microservice de gestion des services a valeur ajoutee parents et etablissements."
        });
    ...
    Log.Information("Services Premium Service demarre sur le port 5010");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Le service Services Premium a echoue au demarrage");
}
finally
{
    Log.CloseAndFlush();

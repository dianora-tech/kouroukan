...
public sealed class ServiceParentControllerTests
{
    ...
    {
        var paged = new PagedResult<ServiceParent>(new List<ServiceParent>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<ServiceParent>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<ServiceParent>>>();
    }
    ...
    {
        var entity = new ServiceParent() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<ServiceParent?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new ServiceParent() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<ServiceParent>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<ServiceParent>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

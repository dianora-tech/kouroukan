...
public sealed class SouscriptionControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<SouscriptionController>> _logger;
    private readonly SouscriptionController _sut;

    public SouscriptionControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<SouscriptionController>>();
        _sut = new SouscriptionController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Souscription>(new List<Souscription>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Souscription>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Souscription>>>();
    }
    ...
    {
        var entity = new Souscription() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Souscription?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Souscription() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Souscription>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Souscription>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

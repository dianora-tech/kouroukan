...
public sealed class AppelControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<AppelController>> _logger;
    private readonly AppelController _sut;

    public AppelControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<AppelController>>();
        _sut = new AppelController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<AppelEntity>(new List<AppelEntity>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<AppelEntity>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<AppelEntity>>>();
    }
    ...
    {
        var entity = new AppelEntity() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<AppelEntity?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new AppelEntity() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<AppelEntity>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<AppelEntity>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

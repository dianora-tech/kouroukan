...
public sealed class AbsenceControllerTests
{
    ...
    {
        var paged = new PagedResult<AbsenceEntity>(new List<AbsenceEntity>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<AbsenceEntity>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<AbsenceEntity>>>();
    }
    ...
    {
        var entity = new AbsenceEntity() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<AbsenceEntity?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<AbsenceEntity?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((AbsenceEntity?)null);
    ...
    {
        var created = new AbsenceEntity() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<AbsenceEntity>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<AbsenceEntity>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

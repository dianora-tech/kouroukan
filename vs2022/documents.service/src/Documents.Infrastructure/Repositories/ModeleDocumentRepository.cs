...
public sealed class ModeleDocumentRepository : IModeleDocumentRepository
{
    private readonly AuditRepository<ModeleDocumentDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public ModeleDocumentRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<ModeleDocumentDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<ModeleDocumentDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<ModeleDocument?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : ModeleDocumentMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<ModeleDocument>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(ModeleDocumentMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        var spec = new SimpleSpecification<ModeleDocumentDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(ModeleDocumentMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<ModeleDocument>(entities, result.TotalCount, result.Page, result.PageSize);
    }

    public async Task<ModeleDocument> AddAsync(ModeleDocument entity, CancellationToken ct = default)
    {
        var dto = ModeleDocumentMapper.ToDto(entity);
        var created = await _repo.AddAsync(dto, ct).ConfigureAwait(false);
        return ModeleDocumentMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM documents.type_modeles_documents WHERE is_deleted = FALSE ORDER BY name";

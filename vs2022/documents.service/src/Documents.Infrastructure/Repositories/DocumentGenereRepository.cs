...
using Documents.Infrastructure.Mappers;
...
public sealed class DocumentGenereRepository : IDocumentGenereRepository
{
    private readonly AuditRepository<DocumentGenereDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public DocumentGenereRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<DocumentGenereDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<DocumentGenereDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<DocumentGenere?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : DocumentGenereMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<DocumentGenere>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(DocumentGenereMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        {
            where = "statut_signature ILIKE @Search OR chemin_fichier ILIKE @Search";
        ...
        var spec = new SimpleSpecification<DocumentGenereDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(DocumentGenereMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<DocumentGenere>(entities, result.TotalCount, result.Page, result.PageSize);
    }

    public async Task<DocumentGenere> AddAsync(DocumentGenere entity, CancellationToken ct = default)
    {
        var dto = DocumentGenereMapper.ToDto(entity);
        var created = await _repo.AddAsync(dto, ct).ConfigureAwait(false);
        return DocumentGenereMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM documents.type_documents_generes WHERE is_deleted = FALSE ORDER BY name";

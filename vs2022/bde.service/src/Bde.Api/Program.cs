...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.Console()
        .Enrich.FromLogContext()
        .Enrich.WithProperty("Application", "bde-service"));
    ...
    builder.Services.AddGnValidation(builder.Configuration,
        typeof(Bde.Application.Validators.CreateAssociationValidator).Assembly);
    ...
    builder.Services.AddMediatR(cfg =>
        cfg.RegisterServicesFromAssembly(typeof(AssociationCommandHandler).Assembly));

    // 9. Domain Services
    builder.Services.AddScoped<IAssociationService, AssociationService>();
    builder.Services.AddScoped<IEvenementService, EvenementService>();
    builder.Services.AddScoped<IMembreBdeService, MembreBdeService>();
    builder.Services.AddScoped<IDepenseBdeService, DepenseBdeService>();

    // 10. Repositories
    builder.Services.AddScoped<IAssociationRepository, AssociationRepository>();
    builder.Services.AddScoped<IEvenementRepository, EvenementRepository>();
    builder.Services.AddScoped<IMembreBdeRepository, MembreBdeRepository>();
    builder.Services.AddScoped<IDepenseBdeRepository, DepenseBdeRepository>();
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "Kouroukan - BDE Service",
            Version = "v1",
            Description = "Microservice de gestion du Bureau des Etudiants : associations, evenements, membres et depenses."
        });
    ...
    Log.Information("BDE Service demarre sur le port 5008");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Le service BDE a echoue au demarrage");
}
finally
{
    Log.CloseAndFlush();

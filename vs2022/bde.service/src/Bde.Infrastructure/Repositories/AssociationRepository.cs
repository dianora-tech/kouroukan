...
{
    private readonly AuditRepository<AssociationDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public AssociationRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<AssociationDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<AssociationDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<Association?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : AssociationMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<Association>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(AssociationMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        {
            where = "name ILIKE @Search OR sigle ILIKE @Search";
        ...
        var spec = new SimpleSpecification<AssociationDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(AssociationMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<Association>(entities, result.TotalCount, result.Page, result.PageSize);
    }

    public async Task<Association> AddAsync(Association entity, CancellationToken ct = default)
    {
        var dto = AssociationMapper.ToDto(entity);
        var created = await _repo.AddAsync(dto, ct).ConfigureAwait(false);
        return AssociationMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM bde.type_associations WHERE is_deleted = FALSE ORDER BY name";

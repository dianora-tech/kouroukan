...
public sealed class EleveControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<EleveController>> _logger;
    private readonly EleveController _sut;

    public EleveControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<EleveController>>();
        _sut = new EleveController(_mediator.Object, _logger.Object);
    ...
    public async Task GetAll_DoitRetournerOk_QuandAppele()
    {
        ...
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Eleve>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Eleve>>>();
    }
    ...
    {
        var entity = new Eleve() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Eleve?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Eleve() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Eleve>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Eleve>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

...
public sealed class InscriptionRepository : IInscriptionRepository
{
    private readonly AuditRepository<InscriptionDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public InscriptionRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<InscriptionDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<InscriptionDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<Inscription?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : InscriptionMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<Inscription>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(InscriptionMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        {
            where = "statut_inscription ILIKE @Search";
        ...
        var spec = new SimpleSpecification<InscriptionDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(InscriptionMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<Inscription>(entities, result.TotalCount, result.Page, result.PageSize);
    }

    public async Task<Inscription> AddAsync(Inscription entity, CancellationToken ct = default)
    {
        var dto = InscriptionMapper.ToDto(entity);
        var created = await _repo.AddAsync(dto, ct).ConfigureAwait(false);
        return InscriptionMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM inscriptions.type_inscriptions WHERE is_deleted = FALSE ORDER BY name";

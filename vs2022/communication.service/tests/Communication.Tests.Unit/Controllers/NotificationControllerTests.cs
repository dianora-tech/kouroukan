...
public sealed class NotificationControllerTests
{
    ...
    {
        var paged = new PagedResult<Notification>(new List<Notification>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Notification>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Notification>>>();
    }
    ...
    public async Task GetById_DoitRetournerOk_QuandEntiteTrouvee()
    {
        ...
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Notification?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Notification() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Notification>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Notification>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

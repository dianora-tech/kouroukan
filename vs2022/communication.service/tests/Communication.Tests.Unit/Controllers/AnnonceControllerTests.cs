...
public sealed class AnnonceControllerTests
{
    ...
    {
        var paged = new PagedResult<Annonce>(new List<Annonce>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Annonce>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Annonce>>>();
    }
    ...
    {
        var entity = new Annonce() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Annonce?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Annonce() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Annonce>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Annonce>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

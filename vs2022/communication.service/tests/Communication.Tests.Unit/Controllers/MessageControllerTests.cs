...
public sealed class MessageControllerTests
{
    ...
    {
        var paged = new PagedResult<Message>(new List<Message>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Message>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Message>>>();
    }
    ...
    public async Task GetById_DoitRetournerOk_QuandEntiteTrouvee()
    {
        ...
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Message?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Message() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Message>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Message>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

...
using Communication.Api.Authorization;
using Communication.Api.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.Console()
        .Enrich.FromLogContext()
        .Enrich.WithProperty("Application", "communication-service"));
    ...
    builder.Services.AddGnValidation(builder.Configuration,
        typeof(Communication.Application.Validators.CreateMessageValidator).Assembly);
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "Kouroukan - Communication Service",
            Version = "v1",
            Description = "Microservice de messagerie interne, notifications push/SMS et annonces."
        });
    ...
    Log.Information("Communication Service demarre sur le port 5007");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Le service Communication a echoue au demarrage");
}
finally
{
    Log.CloseAndFlush();

...
{
    private readonly AuditRepository<NotificationDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public NotificationRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<NotificationDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<NotificationDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<Notification?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : NotificationMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<Notification>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(NotificationMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        var spec = new SimpleSpecification<NotificationDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(NotificationMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<Notification>(entities, result.TotalCount, result.Page, result.PageSize);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM communication.type_notifications WHERE is_deleted = FALSE ORDER BY name";

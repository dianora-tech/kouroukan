...
{
    private readonly AuditRepository<MessageDto> _repo;
    private readonly IDbConnectionFactory _connectionFactory;

    public MessageRepository(
        IDbConnectionFactory connectionFactory,
        ILogger<Repository<MessageDto>> logger,
        IOptions<GnDapperOptions> options,
        IHttpContextAccessor httpContextAccessor)
    {
        _connectionFactory = connectionFactory;
        _repo = new AuditRepository<MessageDto>(connectionFactory, logger, options, httpContextAccessor);
    }
    ...
    public async Task<Message?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        return dto is null ? null : MessageMapper.ToEntity(dto);
    }
    ...
    public async Task<IReadOnlyList<Message>> GetAllAsync(CancellationToken ct = default)
    {
        ...
        return dtos.Select(MessageMapper.ToEntity).ToList().AsReadOnly();
    }
        ...
        int page, int pageSize, string? search, int? typeId, string? orderBy, CancellationToken ct = default)
    {
        ...
        {
            where = "name ILIKE @Search OR sujet ILIKE @Search";
        ...
        var spec = new SimpleSpecification<MessageDto>(
            where, dynamicParams,
        ...
        var entities = result.Items.Select(MessageMapper.ToEntity).ToList().AsReadOnly();
        return new PagedResult<Message>(entities, result.TotalCount, result.Page, result.PageSize);
    }
    ...
    public async Task<Message> AddAsync(Message entity, CancellationToken ct = default)
    {
        ...
        return MessageMapper.ToEntity(created);
    }
    ...
    {
        const string sql = "SELECT id, name, description FROM communication.type_messages WHERE is_deleted = FALSE ORDER BY name";

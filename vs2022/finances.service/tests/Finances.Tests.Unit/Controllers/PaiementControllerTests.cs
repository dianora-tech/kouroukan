...
public sealed class PaiementControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<PaiementController>> _logger;
    private readonly PaiementController _sut;

    public PaiementControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<PaiementController>>();
        _sut = new PaiementController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Paiement>(new List<Paiement>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Paiement>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Paiement>>>();
    }
    ...
    {
        var entity = new Paiement() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Paiement?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Paiement() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Paiement>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Paiement>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

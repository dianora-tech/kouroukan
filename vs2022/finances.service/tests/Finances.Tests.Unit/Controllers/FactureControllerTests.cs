...
public sealed class FactureControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<FactureController>> _logger;
    private readonly FactureController _sut;

    public FactureControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<FactureController>>();
        _sut = new FactureController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Facture>(new List<Facture>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Facture>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Facture>>>();
    }
    ...
    {
        var entity = new Facture() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Facture?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Facture() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Facture>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Facture>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

...
public sealed class SalleControllerTests
{
    ...
    {
        var paged = new PagedResult<Salle>(new List<Salle>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Salle>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Salle>>>();
    }
    ...
    {
        var entity = new Salle() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Salle?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Salle() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Salle>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Salle>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

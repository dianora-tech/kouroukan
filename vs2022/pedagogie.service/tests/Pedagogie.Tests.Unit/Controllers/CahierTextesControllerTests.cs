...
public sealed class CahierTextesControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<CahierTextesController>> _logger;
    private readonly CahierTextesController _sut;

    public CahierTextesControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<CahierTextesController>>();
        _sut = new CahierTextesController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<CahierTextes>(new List<CahierTextes>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<CahierTextes>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<CahierTextes>>>();
    }
    ...
    {
        var entity = new CahierTextes() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<CahierTextes?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new CahierTextes() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<CahierTextes>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<CahierTextes>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

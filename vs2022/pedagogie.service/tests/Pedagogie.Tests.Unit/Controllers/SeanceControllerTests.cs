...
public sealed class SeanceControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<SeanceController>> _logger;
    private readonly SeanceController _sut;

    public SeanceControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<SeanceController>>();
        _sut = new SeanceController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Seance>(new List<Seance>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Seance>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Seance>>>();
    }
    ...
    {
        var entity = new Seance() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Seance?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Seance() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Seance>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Seance>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

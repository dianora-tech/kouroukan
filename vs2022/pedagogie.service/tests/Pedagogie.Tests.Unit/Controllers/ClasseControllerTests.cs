...
public sealed class ClasseControllerTests
{
    private readonly Mock<IMediator> _mediator;
    private readonly Mock<ILogger<ClasseController>> _logger;
    private readonly ClasseController _sut;

    public ClasseControllerTests()
    {
        _mediator = new Mock<IMediator>();
        _logger = new Mock<ILogger<ClasseController>>();
        _sut = new ClasseController(_mediator.Object, _logger.Object);
    ...
    {
        var paged = new PagedResult<Classe>(new List<Classe>(), 0, 1, 20);
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<PagedResult<Classe>>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(paged);

        var result = await _sut.GetAll();

        var ok = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        ok.Value.Should().BeOfType<ApiResponse<PagedResult<Classe>>>();
    }
    ...
    {
        var entity = new Classe() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Classe?>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
    ...
    {
        var created = new Classe() { Id = 1 };
        _mediator.Setup(m => m.Send(It.IsAny<IRequest<Classe>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(created);

        _mediator.Verify(m => m.Send(It.IsAny<IRequest<Classe>>(), It.IsAny<CancellationToken>()), Times.Never());
    }

...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-gray-200 dark:border-gray-700">
          <tr>
            <th class="px-4 py-3 font-medium text-gray-500">{{ $t('enseignant.competences.matiere') }}</th>
            <th class="px-4 py-3 font-medium text-gray-500">{{ $t('enseignant.competences.cycle') }}</th>
            <th class="px-4 py-3 font-medium text-gray-500 w-24" />
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
          <tr v-for="comp in competences" :key="comp.id">
            <td class="px-4 py-3 text-gray-900 dark:text-white">{{ comp.matiere }}</td>
            <td class="px-4 py-3">
              <UBadge :color="cycleColor[comp.cycle] ?? 'info'" variant="subtle" size="sm">
                {{ cycleLabel[comp.cycle] ?? comp.cycle }}
              </UBadge>
            </td>
            <td class="px-4 py-3">
              <UButton variant="ghost" size="xs" color="error" icon="i-heroicons-trash" :loading="saving" @click="removeCompetence(comp.id)" />
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <USlideover v-model:open="showForm">
      <template #header>
        <h3 class="text-lg font-semibold">{{ $t('enseignant.competences.ajouter') }}</h3>
      </template>
      <template #body>
        <div class="space-y-4 p-4">
          <UFormField :label="$t('enseignant.competences.matiere')">
            <UInput v-model="newCompetence.matiere" class="w-full" :placeholder="$t('enseignant.competences.matierePlaceholder')" />
          </UFormField>
          <UFormField :label="$t('enseignant.competences.cycle')">
            <USelect v-model="newCompetence.cycle" class="w-full" :items="[
          ...
          <div class="flex justify-end gap-2">
            <UButton variant="outline" @click="showForm = false">{{ $t('common.cancel') }}</UButton>
            <UButton color="primary" :loading="saving" @click="addCompetence">{{ $t('common.save') }}</UButton>
          </div>
        </div>
      </template>
    </USlideover>

import { useAbonnementStore } from '../stores/abonnement.store'
import type {
  Abonnement,
...
export function useAdminAbonnement() {
  const store = useAbonnementStore()
  ...
  async function create(payload: CreateAbonnementPayload): Promise<Abonnement | null> {
    try {
  ...
  async function update(id: number, payload: UpdateAbonnementPayload): Promise<Abonnement | null> {
    try {

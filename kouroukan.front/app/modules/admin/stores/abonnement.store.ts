...
export const useAbonnementStore = defineStore('admin-abonnement', {
  ...
  actions: {
    ...
    async create(payload: CreateAbonnementPayload): Promise<Abonnement | null> {
      this.saving = true
      try {
        const response = await apiClient.post<Abonnement>(API_PATH, payload)
        if (response.success && response.data) {
          await this.fetchAll()
    ...
    async update(id: number, payload: UpdateAbonnementPayload): Promise<Abonnement | null> {
      this.saving = true
      try {
        const response = await apiClient.put<Abonnement>(`${API_PATH}/${id}`, payload)
        if (response.success && response.data) {
          await this.fetchAll()
          return response.data
        }

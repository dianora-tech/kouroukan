...
export function useAbsence() {
  ...
  async function fetchAll(params?: Partial<AbsenceFilters & { page?: number; pageSize?: number }>): Promise<void> {
    try {
      ...
      toast.add({ title: t('presences.absence.fetchError'), color: 'error' })
    }
  }

  async function fetchById(id: number): Promise<Absence | null> {
    try {
  ...
  async function create(payload: CreateAbsencePayload): Promise<Absence | null> {
    try {
      const result = await store.create(payload)
      if (result) {
        toast.add({ title: t('presences.absence.createSuccess'), color: 'success' })
      }
      return result
    }
    catch {
      toast.add({ title: t('presences.absence.createError'), color: 'error' })
      return null
    }
  }

  async function update(id: number, payload: UpdateAbsencePayload): Promise<Absence | null> {
    try {
      const result = await store.update(id, payload)
      if (result) {
        toast.add({ title: t('presences.absence.updateSuccess'), color: 'success' })
      }
      return result
    }
    catch {
      toast.add({ title: t('presences.absence.updateError'), color: 'error' })
      return null
    }
  }
  ...
  async function remove(id: number): Promise<boolean> {
    try {
      ...
      if (success) {
        toast.add({ title: t('presences.absence.deleteSuccess'), color: 'success' })
      }
      return success
    }
    catch {
      toast.add({ title: t('presences.absence.deleteError'), color: 'error' })
      return false
    }
  }

  function setFilters(filters: AbsenceFilters): void {

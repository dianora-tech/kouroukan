...
export const useNoteStore = defineStore('evaluations-note', {
  state: (): NoteState => ({
    items: [],
    currentItem: null,
    loading: false,
  ...
  getters: {
    ...
    hasFilters: (state): boolean =>
      !!(state.filters.search || state.filters.evaluationId || state.filters.eleveId
        || state.filters.classeId || state.filters.matiereId
  ...
  actions: {
    ...
    async fetchById(id: number): Promise<Note | null> {
      this.loading = true
      try {
        const response = await apiClient.get<Note>(`${API_PATH}/${id}`)
        if (response.success && response.data) {
    ...
    async create(payload: CreateNotePayload): Promise<Note | null> {
      this.saving = true
      try {
        const response = await apiClient.post<Note>(API_PATH, payload)
        if (response.success && response.data) {
          await this.fetchAll()
          return response.data
        }
    ...
    async update(id: number, payload: UpdateNotePayload): Promise<Note | null> {
      this.saving = true
      try {
        const response = await apiClient.put<Note>(`${API_PATH}/${id}`, payload)
        if (response.success && response.data) {
          await this.fetchAll()
          return response.data
        }

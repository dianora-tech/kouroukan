...
const totalPayes = computed(() =>
  props.items.filter(i => i.estPaye).length,
)

const totalRedoublants = computed(() =>
  props.items.filter(i => i.estRedoublant).length,
)

const totalMontant = computed(() =>
  props.items.reduce((sum, i) => sum + i.montantInscription, 0),
)
...
const stats = computed(() => [
  {
    label: t('inscriptions.inscription.stats.total'),
  ...
  {
    label: t('inscriptions.inscription.stats.payes'),
  ...
  {
    label: t('inscriptions.inscription.stats.redoublants'),
    value: String(totalRedoublants.value),
    icon: 'i-heroicons-arrow-path',
    color: 'text-amber-600 dark:text-amber-400',
    bgColor: 'bg-amber-50 dark:bg-amber-900/20',
  },
  {
    label: t('inscriptions.inscription.stats.montantTotal'),

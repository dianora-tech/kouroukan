<script setup lang="ts">
import type { AnneeScolaire } from '../types/annee-scolaire.types'

const props = defineProps<{
  items: AnneeScolaire[]
  totalCount: number
}>()

const { t } = useI18n()

const activeCount = computed(() =>
  props.items.filter(a => a.estActive).length,
)

const lastCreated = computed(() => {
  if (props.items.length === 0) return '-'
  return props.items[0]?.libelle ?? '-'
})

const stats = computed(() => [
  {
    label: t('inscriptions.anneeScolaire.stats.total'),
    value: String(props.totalCount),
    icon: 'i-heroicons-calendar-days',
    color: 'text-green-600 dark:text-green-400',
    bgColor: 'bg-green-50 dark:bg-green-900/20',
  },
  {
    label: t('inscriptions.anneeScolaire.stats.actives'),
    value: String(activeCount.value),
    icon: 'i-heroicons-check-circle',
    color: 'text-blue-600 dark:text-blue-400',
    bgColor: 'bg-blue-50 dark:bg-blue-900/20',
  },
  {
    label: t('inscriptions.anneeScolaire.stats.derniere'),
    value: lastCreated.value,
    icon: 'i-heroicons-clock',
    color: 'text-amber-600 dark:text-amber-400',
    bgColor: 'bg-amber-50 dark:bg-amber-900/20',
  },
])
</script>

<template>
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
    <div
      v-for="stat in stats"
      :key="stat.label"
      class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900"
    >
      <div class="flex items-center gap-3">
        <div class="rounded-lg p-2" :class="stat.bgColor">
          <UIcon :name="stat.icon" class="h-5 w-5" :class="stat.color" />
        </div>
        <div>
          <p class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ stat.value }}
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ stat.label }}
          </p>
        </div>
      </div>
    </div>
  </div>
</template>
